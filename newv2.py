...
# API endpoints (UPDATED: Added family API)
APIS = {
    # Existing APIs
    "num": "https://aetherosint.site/cutieee/api.php?key=ENCORE&type=mobile&term==",
    "aadhar": "https://aetherosint.site/cutieee/api.php?key=ENCORE&type=id_number&term=",
    ...
    "gst": "https://gst-info.vercel.app/CHIRAGX9/",
    ...
    "vnum": "https://api.x10.network/numapi.php?action=api&key=xdhyszionx&test1=",
    "rto": "https://api.x10.network/numapi.php?action=api&key=xdhyszionx&test1=",
    "leak": "https://aluuxidk.vercel.app/leak?key=6442851093:7ocdcXMi&id=",
    "imei": "https://imei-info.gauravyt566.workers.dev/?imei=",
    "family": "https://api.x10.network/numapi.php?action=api&key=xdhyszionx&aadhar_family=",  # ADD YOUR FAMILY API HERE
}
...
def init_database():
    ...
    # Create groups table WITHOUT welcome_enabled column (REMOVED)
    cursor.execute('''
...
async def check_min_group_members(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check if group has minimum required members before allowing commands"""
...
def validate_query(service: str, query: str) -> Tuple[bool, str]:
    """Validate query format before sending to API"""
    query = query.strip()
    
    if service == "num":
        # Phone number validation (Indian numbers: 10 digits)
        ...
        if not query.isdigit() or len(query) != 12:
        ...
        if len(query) < 5 or len(query) > 15:
            return False, "🚗 Invalid vehicle number format"
        return True, query
    
    elif service == "tg":
        # Telegram username/ID validation
        if query.startswith('@'):
            query = query[1:]  # Remove @ symbol
        if not query:
            return False, "📱 Please provide Telegram userid"
        return True, query
    
    elif service == "insta":
        # Instagram username validation
        if not query:
            return False, "📸 Please provide Instagram username"
        return True, query
    ...
    elif service == "ffuid" or service == "ffinfo":
        # Free Fire UID validation (numbers)
        if not query.isdigit():
            return False, "🎮 Free Fire UID must contain only numbers"
        return True, query
    
    elif service == "pak":
        # Pakistan number validation
        if not query.isdigit() or len(query) < 10:
            return False, "🇵🇰 Pakistan number must be at least 10 digits"
        return True, query
    
    elif service == "vnum" or service == "rto":
        ...
        if not query.isdigit() or len(query) != 15:
            return False, "📱 IMEI must be 15 digits"
        return True, query
    
    elif service == "family":
        # Family search validation
        if len(query) < 12:
            return False, "👪 Search query too short"
        return True, query
...
"""
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("ᴅᴇᴠᴇʟᴏᴩᴇʀ", url="https://zionixpy.t.me")
        ]
    ]
    ...
    try:
        ...
        # If result was truncated, send full JSON as file (BOTH IN GROUPS AND PRIVATE)
...
async def pak_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_min_group_members(update, context):
        return
...
async def pak_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, phone: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("pak", phone)
    if not is_valid:
...
async def ffinfo_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("ffinfo", uid)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def vnum_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, vehicle_no: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("vnum", vehicle_no)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def rto_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, vehicle_no: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("rto", vehicle_no)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def leak_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, leak_id: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("leak", leak_id)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def imei_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, imei_no: str):
    """Handle IMEI lookup"""
    # Validate query
    is_valid, error_msg = validate_query("imei", imei_no)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def family_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /family command for family tree lookup"""
    if not await force_join_check(update, context):
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "ᴘʟᴇᴀꜱᴇ ᴘʀᴏᴠɪᴅᴇ ꜰᴀᴍɪʟʏ ꜱᴇᴀʀᴄʜ ǫᴜᴇʀʏ (ɴᴀᴍᴇ, ᴘʜᴏɴᴇ, ᴀᴀᴅʜᴀʀ).\nᴜꜱᴀɢᴇ: <code>/family rahul</code>",
            parse_mode=ParseMode.HTML
        )
        return
    
    family_query = context.args[0]
    await family_command_handler(update, context, family_query)

async def family_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, family_query: str):
    """Handle family tree lookup"""
    # Validate query
    is_valid, error_msg = validate_query("family", family_query)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
    
    try:
        if APIS['family'] == "YOUR_FAMILY_API_ENDPOINT_HERE":
            await update.message.reply_text(
                "❌ <b>Family API not configured.</b>\n\nPlease add your family API endpoint to the APIS dictionary.",
                parse_mode=ParseMode.HTML
            )
            return
            
        response = requests.get(f"{APIS['family']}{family_query}", timeout=10)
        if response.status_code == 200:
            data = response.json()
            await send_api_result(update, context, "family", family_query, data)
        else:
            raise Exception("API Error")
    except Exception as e:
        await update.message.reply_text(
...
"""
        ...
        if chat.type == "private":
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton("👪 ꜰᴀᴍɪʟʏ", callback_data="service_family"),
...
<code>/family [ǫᴜᴇʀʏ]</code> - ꜰᴀᴍɪʟʏ ᴛʀᴇᴇ ʟᴏᴏᴋᴜᴘ
...
"""
        ...
        services = {
            ...
            "tg": "📱 ᴛᴇʟᴇɢʀᴀᴍ ᴜꜱᴇʀ ɪɴꜰᴏ\n\nᴘʟᴇᴀꜱᴇ ʀᴇᴘʟʏ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴍ ᴜꜱᴇʀɴᴀᴍᴇ/ɪᴅ (ᴡɪᴛʜᴏᴜᴛ @):",
            ...
            "family": "👪 ꜰᴀᴍɪʟʏ ᴛʀᴇᴇ ʟᴏᴏᴋᴜᴘ\n\nᴘʟᴇᴀꜱᴇ ʀᴇᴘʟʏ ᴡɪᴛʜ ꜰᴀᴍɪʟʏ ꜱᴇᴀʀᴄʜ ǫᴜᴇʀʏ (ɴᴀᴍᴇ, ᴘʜᴏɴᴇ, ᴀᴀᴅʜᴀʀ):"
        }
...
async def handle_service_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'waiting_for' in context.user_data and update.message.text:
        ...
        # Check group member count for group chats
        ...
        if service == "num":
            ...
            await family_command_handler(update, context, query)
...
async def num_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, phone: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("num", phone)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def aadhar_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, aadhar_id: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("aadhar", aadhar_id)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def ip_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, ip_address: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("ip", ip_address)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def vehicle_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, vehicle_no: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("vehicle", vehicle_no)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def tg_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, username: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("tg", username)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def insta_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, username: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("insta", username)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def gst_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, gst_no: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("gst", gst_no)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def ifsc_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, ifsc_code: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("ifsc", ifsc_code)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def mail_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, email: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("mail", email)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
async def ffuid_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, ffuid: str):
    ...
    # Validate query
    is_valid, error_msg = validate_query("ffuid", ffuid)
    if not is_valid:
        await update.message.reply_text(f"❌ {error_msg}", parse_mode=ParseMode.HTML)
        return
...
• /family [ǫᴜᴇʀʏ] - ꜰᴀᴍɪʟʏ ᴛʀᴇᴇ ʟᴏᴏᴋᴜᴘ
...
<b>⚠️ ɴᴏᴛᴇ:</b>ᴜꜱᴇ ɪᴛ ꜰᴏʀ ᴇᴅᴜᴄᴀᴛɪᴏɴᴀʟ ᴩᴜʀᴩᴏꜱᴇꜱ ᴏɴʟʏ
...
async def post_init(application: Application):
    ...
    commands = [
        ...
        BotCommand("family", "ꜰᴀᴍɪʟʏ ᴛʀᴇᴇ ʟᴏᴏᴋᴜᴘ"),
